...
| B- | 80-82 | Significant gaps | Iterate urgently, flag to [Your name] |
| C | 70-79 | Major problems | Escalate to [Your name] |
| F | 0-69 | Fundamental rework needed | Escalate to [Your name] immediately |

## Threshold Rules

- **Ship threshold: A- (90)** — Output can go to [Your name] or next pipeline stage
- **Iteration budget: 3 rounds max** — If still below A- after 3 iterations, escalate
- **Auto-escalate: Below B-** — Don't iterate, go straight to [Your name]
- **[Your agent name] (orchestrator) decides** whether to iterate or escalate. Validators grade, they don't decide.
...
After every grade, the orchestrator ([Your agent name]) appends an entry to `state/evals.yaml`:
...
- `outcome` is one of: `shipped` (≥A-), `iterated` (sent back), `escalated` (to [Your name])
- [Your agent name] logs this, not the critic (critics grade, [Your agent name] records)
- Daily Reflection aggregates stats nightly

## Integration

- All validators reference this file: `Read agents/_shared/grading-protocol.md`
- [Your agent name] uses the grade to decide: ship / iterate / escalate
