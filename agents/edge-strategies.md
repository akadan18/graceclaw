...
**Use when:** Validator needs to assess the thinking, not just the result. Domain critics reviewing creator work.
...
| Any → User (escalate) | `summary` + link | User gets the decision point, not a wall of text |
...
- **Dumping everything by default** — Wastes tokens, can bias critics
